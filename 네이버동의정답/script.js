...
$('.total label').on('click' , function(){
  if($(this).find('.checkbox-img').hasClass('checked')) {
    ...
    $('.agree').find('input[type="checkbox"]').removeAttr('checked','checked');
  }
});
...
//#submit 버튼을 클릭 했을때 필수동의 사항에 모두 체크했는지 확인
//모두 체크되어 있으면 통과
//체크 안된게 있다면 #red-alert을 보여주고 전송을 중지
